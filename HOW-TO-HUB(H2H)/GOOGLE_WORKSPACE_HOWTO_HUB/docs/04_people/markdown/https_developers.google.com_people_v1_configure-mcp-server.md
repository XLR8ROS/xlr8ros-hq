...
- <div class="devsite-breadcrumb-guillemet material-icons" aria-hidden="true">
  ...
  <a href="https://developers.google.com/workspace" class="devsite-breadcrumb-link gc-analytics-event" data-category="Site-Wide Custom Events" data-label="Breadcrumbs" data-value="3" data-track-type="globalNav" data-track-name="breadcrumb" data-track-metadata-position="3" data-track-metadata-eventdetail="Google Workspace">Google Workspace</a>

- <div class="devsite-breadcrumb-guillemet material-icons" aria-hidden="true">

  </div>

  <a href="https://developers.google.com/people" class="devsite-breadcrumb-link gc-analytics-event" data-category="Site-Wide Custom Events" data-label="Breadcrumbs" data-value="4" data-track-type="globalNav" data-track-name="breadcrumb" data-track-metadata-position="4" data-track-metadata-eventdetail="People API">People API</a>

- <div class="devsite-breadcrumb-guillemet material-icons" aria-hidden="true">

  </div>

  <a href="https://developers.google.com/people/v1/configure-mcp-server" class="devsite-breadcrumb-link gc-analytics-event" data-category="Site-Wide Custom Events" data-label="Breadcrumbs" data-value="5" data-track-type="globalNav" data-track-name="breadcrumb" data-track-metadata-position="5" data-track-metadata-eventdetail="">MCP server</a>

</div>

# Configure the People API MCP server <span slot="popout-heading"> Stay organized with collections </span> <span slot="popout-contents"> Save and categorize content based on your preferences. </span>
...
People API offers a remote [Model Context Protocol (MCP)](https://modelcontextprotocol.io/introduction) server that allows AI agents to securely interact with Google Workspace profile and contact data. By configuring the People API MCP server, you enable AI applications like Gemini CLI, Claude, or IDEs to perform actions using the People API.

The People API MCP server provides a standardized way for AI agents to:
...
### Enable the API and MCP service

To use the People API MCP server, you must enable the following service in your Google Cloud project:
...
<a href="https://console.cloud.google.com/flows/enableapi?apiid=people.googleapis.com" class="button button-primary" target="console">Enable the APIs</a>
...
The People API MCP server uses OAuth 2.0 for authentication and authorization. You must configure the OAuth consent screen before you can create an OAuth client ID.
...
2.  If you have already configured the **Google Auth Platform**, you can configure the following OAuth Consent Screen settings in [Branding](https://console.cloud.google.com/auth/branding), [Audience](https://console.cloud.google.com/auth/audience), and [Data Access](https://console.cloud.google.com/auth/scopes). If you see a message that says **Google Auth Platform not configured yet**, click **Get Started**:
    ...
    1.  Under **App Information**, in **App name**, type `People API MCP Server`.
...
3.  Click **Data Access** **\>** **Add or Remove Scopes**. A panel appears with a list of scopes for each API that you've enabled in your Google Cloud project.
    ...
    1.  Under **Manually add scopes**, paste the scopes for the People API MCP server:
...
To add the People API remote MCP server to your MCP client, follow the instructions for your client.
...
To add the People API remote MCP server to your Gemini CLI, add the server configuration to your `settings.json` file.
...
To use the People API remote MCP server with Claude.ai or Claude Desktop, you must have the Claude Enterprise, Pro, Max, or Team plan.
...
2.  Configure the MCP server in Claude:
    ...
    3.  Enter the connection details for the People API product:
        - **Server name**: `people`.
...
Many AI applications have ways to connect to a remote MCP server. You typically need to enter details about the server, like its name, endpoints, transport protocol, and authentication method. For the People API remote MCP server, enter the following:
...
- **Authentication**: The People API remote MCP server uses OAuth 2.0. For details, see [Learn about authentication and authorization](/workspace/guides/auth-overview).
...
## Test the People API MCP server
...
If the tools execute successfully and you receive relevant responses, your People API MCP server is correctly configured.
...
The following tools are available for the People API MCP server:
...
When you expose a language model to untrusted data, there is a risk of an [indirect prompt injection attack](https://en.wikipedia.org/wiki/Prompt_injection). Because MCP clients like Gemini CLI have access to powerful tools and APIs through the People API MCP server, they can read data in your Google Account.

To mitigate these risks, follow these best practices:

- **Only use trusted tools.** Never connect People API MCP server to untrusted or unverified applications.
- **Be cautious with untrusted inputs.** Avoid asking your MCP client to process resources from unverified sources. These inputs may contain hidden instructions that can hijack your session, allowing an attacker to modify, steal, or delete your data.
...
- [Configure the Drive MCP server](/workspace/drive/api/guides/configure-mcp-server)
