...
## 6.1 Commit classification

Before committing local changes, classify the working tree by path and change type. Safe routine changes may be committed together when they match the assigned workflow. Broad deletions, file moves, protected-doc changes, memory-lane changes, config/auth changes, generated archive material, or unclear diffs require a separate report before they are treated as routine.

A commit report should state the repo, commit hash, paths committed, paths left uncommitted, and any questionable changes held back.
...
When reporting commit work, include the commit hash and the classification of any held-back or questionable changes.